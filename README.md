...
- `backend/`: 🛠️ Express + Prisma API used by the app.
- `schemas/`: 📐 Shared Zod schemas used across frontend and backend.
- `config/`, `docker/`, `k8s/`: ⚙️ Environment, Docker Compose, and Kubernetes setup.
...
- Backend: 🛠️ Node.js, Express 5, Prisma, PostgreSQL, Passport JWT/Local, Zod
...
- `Docker Desktop`
- `Git`
- `ngrok` account/domain (for testing on physical devices)
- `Expo Go` app on your phone (optional, for mobile testing)

## 🔐 Accounts And Access You Need

Before your first run, make sure you also have:

- Access to this GitHub repository
- Docker Desktop permission to run containers locally
- A valid `NGROK_AUTHTOKEN` in `config/environment-variables/.env.grok.dev`
- Local `.env` files created via `npm run create-envs`
- Network access to pull Docker images and npm packages
...
Run these from the repository root to confirm your machine is ready.
...
docker info | Out-Null; if ($LASTEXITCODE -eq 0) { "Docker is running" } else { "Start Docker Desktop" }

# 3) Required env files exist (after `npm run create-envs`)
Test-Path "config/environment-variables/.env.db.dev"
Test-Path "config/environment-variables/.env.backend.dev"
Test-Path "config/environment-variables/.env.grok.dev"
Test-Path "config/environment-variables/.env.docker.dev"

# 4) App env exists
Test-Path "app/.env"

# 5) ngrok auth token is set
Select-String -Path "config/environment-variables/.env.grok.dev" -Pattern "NGROK_AUTHTOKEN"
```
...
# 2) Docker daemon availability
docker info > /dev/null && echo "Docker is running" || echo "Start Docker Desktop / Docker Engine"

# 3) Required env files exist (after `npm run create-envs`)
test -f config/environment-variables/.env.db.dev && echo ".env.db.dev OK" || echo ".env.db.dev missing"
test -f config/environment-variables/.env.backend.dev && echo ".env.backend.dev OK" || echo ".env.backend.dev missing"
test -f config/environment-variables/.env.grok.dev && echo ".env.grok.dev OK" || echo ".env.grok.dev missing"
test -f config/environment-variables/.env.docker.dev && echo ".env.docker.dev OK" || echo ".env.docker.dev missing"

# 4) App env exists
test -f app/.env && echo "app/.env OK" || echo "app/.env missing"

# 5) ngrok auth token is set
grep -n "NGROK_AUTHTOKEN" config/environment-variables/.env.grok.dev
```

If all commands return expected output, continue with:
...
4. ▶️ Start full dev setup (backend containers + Expo app):
